...
arxiv_researcher_agent = Agent(
    ...
    tools=[arxiv_search],
    backstory=(
        "You are an expert in scientific literature retrieval. Your role is to identify the most relevant and impactful papers from arXiv. "
        "You specialize in crafting optimal queries based on user-specified topics, overfetching relevant research to ensure thorough coverage, "
...
def run_literature_review(topic: str, num_papers: int):
    ...
    result = crew.kickoff()
    # return result
    # Manually parse the raw output as JSON
    try:
